...
=======
    ...
    enrolled_courses = db.session.query(
        Course, Enrollment, University
    ).join(
        Enrollment, Course.course_id == Enrollment.course_id
    ).join(
        University, Course.uni_id == University.uni_id
    ).filter(
...
def explore_courses():
    ...
    enrolled_course_ids = db.session.query(
